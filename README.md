...
- Nếu chạy môi trường cục bộ (Local): Cần cài đặt [Node.js](https://nodejs.org/) và [Git](https://git-scm.com/).
...
1. Truy cập trình duyệt và mở [Remix IDE](https://remix.ethereum.org/).
2. Tại tab **File Explorer** (biểu tượng tệp tin ở góc trên bên trái), tạo một Workspace mới hoặc dùng mặc định.
3. Bấm vào nút **Upload file** (biểu tượng tờ giấy có mũi tên hướng lên).
4. Tìm đến thư mục `contracts` trong máy tính mà bạn vừa giải nén, bôi đen toàn bộ các file `.sol` (bao gồm `VotingCore.sol`, `CandidateManager.sol`, `VoterRegistry.sol`, `ElectionScheduler.sol`) và bấm Open để tải lên.

**Bước 3: Biên dịch hợp đồng (Compile)**
1. Bấm đúp chuột để mở file `VotingCore.sol` trên Remix.
2. Chuyển sang tab **Solidity Compiler** (biểu tượng chữ `S` ở menu thanh dọc ngoài cùng bên trái).
3. Đảm bảo mục **Compiler** chọn phiên bản phù hợp (ví dụ: `0.8.24` hoặc khớp với dòng `pragma solidity` trong code).
4. Bấm nút màu xanh dương **"Compile VotingCore.sol"**.
5. *(Nếu thành công, tab Compiler sẽ hiện biểu tượng dấu tích xanh lá cây, không có dòng báo lỗi màu đỏ).*

**Bước 4: Triển khai hợp đồng (Deploy)**
1. Chuyển sang tab **Deploy & Run Transactions** (biểu tượng logo Ethereum ở thanh bên trái).
2. Tại mục **Environment**, chọn **Remix VM (Cancun)** hoặc **Remix VM (Shanghai)**. (Đây là mạng blockchain ảo đã có sẵn 100 ETH giả để làm phí gas).
3. Tại mục **Contract**, bấm vào menu thả xuống và tìm chọn chính xác hợp đồng **`VotingCore`** (vì đây là hợp đồng tổng chứa các module khác).
4. Bấm nút màu cam **"Deploy"**.
5. Kéo xuống mục **Deployed Contracts** ở bên dưới, bấm mũi tên mở rộng để thấy các nút chức năng. Bây giờ bạn có thể tương tác thực tế (Thêm ứng viên, đăng ký cử tri, bỏ phiếu) trực tiếp trên giao diện!
